...
현재 버전은 `v0.13`입니다. 실제 운영 체크리스트 117건은 `업무목록.csv`를 정본으로 사용하고, `data/checklist-metadata.json`이 각 CSV key에 일정·단계·업무 분류 정보를 연결합니다. 업무관리는 체크리스트·현재 구간·진행 현황을 하나의 압축 화면으로 제공하며, `data/tasks.json`은 별도의 검증용 20건 업무 마스터와 편집 화면 원본으로 유지합니다. 담당자 교체 시에는 `인수인계` 화면에서 현재 진행구간, 우선 확인 업무, 메모, 최근 처리, 구간별 현황을 확인하고 읽기용 HTML 보고서를 만들 수 있습니다. AI 정밀분석은 키를 서버에만 둔 Node.js 서버를 통해 OpenAI Responses API와 연결합니다.
...
상단 메뉴는 `업무관리`, `인수인계`, `AI 누락점검`, `예산·정산`, `관리`의 다섯 항목입니다. `관리`에서 업무 마스터, 교육일정 설정, 데이터 관리를 엽니다. 업무관리 화면은 CSV 원본 순서의 단일 목록과 검색·구간·상태·정렬 도구를 제공하며, 행을 열어 3회 체크와 인수인계 메모를 확인하거나 입력합니다. 업무 마스터와 AI·예산 화면은 모달이 아닌 독립 페이지로 전환됩니다.
