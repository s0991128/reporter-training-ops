...
상단 메뉴는 `교육운영`, `교육일정`, `예산·정산`, `더보기`의 네 항목입니다. 더보기에서 업무기준 관리, 인수인계, AI 누락점검, 데이터 관리를 엽니다. 교육운영 화면은 현재 진행 구간을 기본으로 표시하고, 16개 구간 가로 단계 내비게이션·우선 확인 업무·검색·상태 필터를 제공합니다. 전체업무를 선택하면 CSV 원본 순서의 117건을 구간별 컴팩트 목록으로 확인할 수 있습니다. 행을 열어 3회 체크와 인수인계 메모를 확인하거나 입력합니다.
