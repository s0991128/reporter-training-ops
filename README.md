...
상단 메뉴는 `교육운영`, `교육일정`, `예산·정산`, `더보기`의 네 항목입니다. 더보기에서 업무기준 관리, 인수인계, AI 누락점검, 데이터 관리를 엽니다. 교육운영 화면은 현재 진행 구간을 기본으로 표시하고, 현재 구간·진행률·완료·진행중·미완료 KPI, 이전·현재·다음 구간과 전체 구간 선택, 우선 확인 업무, 검색·상태 필터를 제공합니다. 현재 구간 업무가 4건보다 적으면 다음 구간 업무를 필요한 수만큼 미리보기로 덧붙여 첫 화면에서 실제 업무를 바로 확인할 수 있습니다. 전체업무를 선택하면 CSV 원본 순서의 117건을 구간별 컴팩트 목록으로 확인할 수 있습니다. 행을 열어 3회 체크와 인수인계 메모를 확인하거나 입력합니다.
