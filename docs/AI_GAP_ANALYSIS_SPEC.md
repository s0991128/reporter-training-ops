...
과거 교육계획서, 체크리스트, 인수인계 자료에서 업무성 문장을 추출하고 현재 117건 운영 task와 비교해 누락·보강·이미 반영 후보를 제안합니다. `data/tasks.json`은 업무 마스터 편집 후보를 검증하는 별도 샘플 기준입니다. 분석결과는 담당자의 검토를 돕는 참고자료이며 AI가 업무를 확정하지 않습니다.
